...
int main(int argc, char **argv) {
 ...
 if (!p) {    // if child
   ...
   for (int i = 0; i < NUM_LOOP1; i++) {
    ...
    if (setpriority(pid, me * 2) < 0) {
      printf(1, "setpriority failure: %d, pid: %d, me*2: %d\n", setpriority(pid, me*2), pid, me*2);
    }
   }
 ...
 if (!p) {
   int pid = getpid();
   int cnt[2] = {0};
   if (me == NUM_CHILD - 1)       // last forked process
     monopolize(2019011449);      // monopolize mode
   for (int i = 0; i < NUM_LOOP4; i++)
     cnt[getlev()]++;
   printf(1, "process %d: L0=%d, L1=%d\n", pid, cnt[0], cnt[1]);
   if (me == NUM_CHILD - 1)
     monopolize(2015011449);
 }
